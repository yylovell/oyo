...
class Events
{
    ...
    {
        if (empty(self::$db)) {
            self::$db = new \Workerman\MySQL\Connection('127.0.0.1', '3306', 'root', '', 'oyo');
        }
        ...
        switch ($message['type']) {
            ...
            case 'chatMessage':
                ...
                if(!empty($client)){
                    ...
                    self::$db->insert('oyo_cus_chat_log')->cols($serviceLog)->query();
                    unset($serviceLog);
                }
                break;
            // 转接
            case 'changeGroup':
                ...
                // 记录该客服与该会员的服务结束
                self::$db->query("update `oyo_cus_service_log` set `end_time` = " . time() . " where `client_id`= '" . $userClient . "'");

                // 从当前客服的服务表中删除这个会员
                $old = $kfList = self::$global->kfList;
                if(!isset($kfList[$userGroup])){
                    $waitMsg = '暂时没有相关客服在线,请稍后再咨询。';
                    // 逐一通知
        ...
        foreach($kfList as $k=>$v){
            foreach($v as $key=>$vo){
                if(in_array($client_id, $vo['user_info'])){
                    ...
                    self::$db->query("update `oyo_cus_service_log` set `end_time` = " . time() . " where `client_id`= '" . $client_id . "'");
                    ...
                    //$outUser = self::$db->query("select `user_id`,`group_id` from `oyo_cus_service_log` where `client_id`= '" . $client_id . "'");
                    // 通知 客服删除退出的用户
                    if(!empty($outUser)){
                        $del_message = [
    ...
    private static function userOnlineTask($client_id, $group)
    {
        ...
        if(1 == $res['code']){
            ...
            // 检测是否开启自动应答
            $sayHello = self::$db->query('select `word`,`status` from `oyo_cus_reply` where `id` = 1');
            if(!empty($sayHello) && 1 == $sayHello['0']['status']){
                $kfId = substr($res['data'][0], 2);
                $cusData = self::$db->query('select `user_avatar` from `oyo_customer_service` where `id` = ' . $kfId);
                $hello = [
                    'message_type' => 'helloMessage',
                    'data' => [
                        'name' => $res['data']['1'],
                        'avatar' => $cusData['0']['user_avatar'],
                        'id' => $res['data']['0'],
                        'time' => date('H:i'),
            ...
            self::$db->insert('oyo_cus_service_log')->cols($serviceLog)->query();
            unset($serviceLog);
    ...
    {
        $maxNumber = self::$db->query('select `max_service` from `oyo_kf_config` where `id` = 1');
        if(!empty($maxNumber)){
            $maxNumber = 5;
        }else{
    ...
    private static function writeLog($flag = 1)
    {
        ...
        self::$db->update('oyo_cus_now_data')->cols($param)->where('id=1')->query();
        ...
        if(2 == $flag){
            ...
            self::$db->insert('oyo_cus_service_data')->cols($param)->query();
        }
