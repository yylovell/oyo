...
layui.use(['upload'], function () {
    ...
    var uploadInstImg = upload.render({
        ...
        , done: function (res) {
            ...
            sendMsg('img[/../../public/uploads/' + res.data.src + ']');
        }
        , error: function () {
            // 请求异常回调
        }
...
function sendMsg(sendMsg){
    ...
    _html += '<img src="../../public/uploads/' + config.avatar + '"><cite>我</cite></div>';
...
function showMsg(info, flag){
    ...
    _html += '<div class="layim-chat-user">';
    _html += '<img src="../../public/uploads/' + info.avatar + '"><cite>' + info.name + '</cite></div>';
    _html += '<div class="layim-chat-text">' + content + '</div></li>';
...
function showFaces(){
    ...
    for(var index = 0; index < len; index++){
        _html += '<li title="' + alt[index] + '" onclick="checkFace(this)"><img src="../../public/static/cus/images/face/'+ index + '.gif" /></li>';
    }
    _html += '</ul>';
...
function showChatLog(){
    ...
    for(var i = 0; i < len; i++){
        ...
        if('mine' == item.type){
            ...
            _html += '<li class="layim-chat-li layim-chat-mine">';
            _html += '<div class="layim-chat-user">';
            _html += '<img src="../../public/uploads/' + item.avatar + '"><cite>'+ item.name + '</cite></div>';
