...
$(function () {
    ...
    $.getJSON('index/getUserList', function(res){
        if(1 == res.code && res.data.length > 0){
            ...
            $.getJSON('getCity', {ip: ip}, function(res){
                $("#f-area").val(res.data);
            });
...
layui.use(['upload', 'layer'], function () {
    ...
    var uploadInstImg = upload.render({
        ...
        , done: function (res) {
            ...
            sendMessage('img[/../../public/uploads/' + res.data.src + ']');
            showBigPic();
        }
        , error: function () {
            // 请求异常回调
...
function showFaces() {
    ...
    layui.each(alt, function (index, item) {
        _html += '<li title="' + item + '" onclick="checkFace(this)"><img src="../../public/static/cus/js/layui/images/face/' + index + '.gif" /></li>';
    });
    _html += '</ul></div>';
...
function msgFactory(content, type, uinfo) {
    ...
    _html += '<img src="../../public/uploads/' + uinfo.avatar + '">';
    if ('mine' == type) {
...
function addUser(data) {
    ...
    _html += '<img src="../../public/uploads/' + data.avatar + '">';
